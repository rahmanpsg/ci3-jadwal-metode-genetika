...
{    
    ...
    function __construct($jurusan, $jenis_semester, $tahun_akademik, $populasi, $crossOver, $mutasi, $kode_jumat, $range_jumat, $kode_dhuhur)
    {        
        parent::__construct();        
        ...
        $this->kode_dhuhur    = explode('-',$kode_dhuhur);
    ...
    public function AmbilData()
    {
        ...
        $rs_data = $this->admin_models->ambilDataGenerateJadwal($this->jenis_semester,$this->tahun_akademik,$this->jurusan);
        ...
        foreach ($rs_hari->result() as $data) {
            $this->hari[$i] = array('id'=>intval($data->id),'kelas'=>$data->kelas);
            $i++;
        }
        ...
        $rs_RuangReguler = $this->db->query("SELECT id FROM tbl_ruang WHERE jenis = '$this->TEORI'");
        $i               = 0;
        ...
        for ($i = 0; $i < $this->populasi; $i++) {
            ...
            for ($j = 0; $j < $jumlah_pengampu; $j++) {
                ...
                if ($this->jenis_kuliah[$j] === $this->REGULER) {
                    $this->individu[$i][$j][2] = array_rand($this->kelashariReguler, 1) - 1; // Penentuan hari secara acak untuk kelas REGULER    
                }else{
    ...
    private function CekFitness($indv)
    {
        ...
        $jumat_3 = intval($this->range_jumat[3]);
        ...
        for ($i = 0; $i < $jumlah_pengampu; $i++)
        {
          ...
          $kelas_a = $this->kelas[$i];
            ...
            if (($hari_a  + 1) == $hari_jumat) //2.bentrok sholat jumat
            {
                ...
                for ($d=1; $d < $sks; $d++) { 
                    if (($jam_a == ($jumat_0 - $d-1))) {
                        $penalty += 1;
                    }
                }
                if (($jam_a == ($jumat_0 - 1)) || ($jam_a == ($jumat_1 - 1)) || ($jam_a == ($jumat_2 - 1)) || ($jam_a == ($jumat_3 - 1))){
                    $penalty += 1;                        
                }
            }else{ // Bentrok Sholat Dhuhur
                for ($d=1; $d < $sks; $d++) { 
                    if (($jam_a == ($this->kode_dhuhur[0] - $d-1))) {
                        $penalty += 1;
                    }
                }
                if (($jam_a == ($this->kode_dhuhur[0] - 1)) || ($jam_a == ($this->kode_dhuhur[1] - 1)) || ($jam_a == ($this->kode_dhuhur[2] - 1)))
                {                
                    $penalty += 1;
                }
            }
        ...
        for ($i = 0; $i < $this->populasi; $i++) {
            ...
            if ($r < $this->mutasi) {
                ...
                if ($this->jenis_kuliah[$krom] === $this->REGULER) {
                    $this->individu[$i][$krom][2] = array_rand($this->kelashariReguler, 1) - 1; // Penentuan hari secara acak untuk kelas REGULER    
                }else{
